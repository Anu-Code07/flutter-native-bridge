...
class BridgeEvent {
  ...
  const BridgeEvent({this.name, this.replay = 0, this.bufferSize = 64});
...
class BridgeSerializable {
  ...
  const BridgeSerializable({this.discriminator, this.serializer});

  final String? discriminator;
  final Type? serializer;
}

enum BridgeCodec { json, binary, protobuf, custom }

enum BridgePlatform { android, ios, macos, windows, linux }

enum BridgeTransport { methodChannel, eventChannel, basicMessageChannel, ffi }

enum FFIThreading { main, worker, isolate }
